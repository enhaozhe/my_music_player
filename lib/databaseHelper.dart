...
// Helper methods
  ...
  Future<int> updateRow(Map<String, dynamic> row) async {
    Database db = await instance.database;
    return await db.rawUpdate('''UPDATE $table SET $columnTitle = ?, $columnArtist = ?, $columnAlbum = ? WHERE $columnId = ?''',
        [row[columnTitle], row[columnArtist], row[columnAlbum], row[columnId]]);
  }
